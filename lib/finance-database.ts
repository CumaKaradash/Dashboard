...
class FinanceDatabase {
  ...
  // Payment operations
  payments = {
    getAll: (): Payment[] => this.payments,
    getById: (id: string): Payment | undefined => this.payments.find((p) => p.id === id),
    getByPatient: (patientId: string): Payment[] => this.payments.filter((p) => p.patientId === patientId),
    getByDate: (date: string): Payment[] => this.payments.filter((p) => p.date === date),
    getByStatus: (status: string): Payment[] => this.payments.filter((p) => p.status === status),
  ...
  // Invoice operations
  invoices = {
    getAll: (): Invoice[] => this.invoices,
    getById: (id: string): Invoice | undefined => this.invoices.find((i) => i.id === id),
    getByPatient: (patientId: string): Invoice[] => this.invoices.filter((i) => i.patientId === patientId),
    getByStatus: (status: string): Invoice[] => this.invoices.filter((i) => i.status === status),
  ...
  // Budget operations
  budgets = {
    getAll: (): Budget[] => this.budgets,
    getById: (id: string): Budget | undefined => this.budgets.find((b) => b.id === id),
    getByPeriod: (period: string, year: number, month?: number): Budget[] => {
      return this.budgets.filter((b) => b.period === period && b.year === year && (!month || b.month === month))
    },
  ...
  calculations = {
    getTotalRevenue: (startDate?: string, endDate?: string): number => {
      let payments = this.payments.filter((p) => p.status === "completed")
    ...
    getOverdueInvoices: (): Invoice[] => {
      ...
      return this.invoices.filter((i) => i.status === "sent" && i.dueDate < today)
    },
    getBudgetUtilization: (categoryId: string): number => {
      const budget = this.budgets.find((b) => b.id === categoryId)
      if (!budget) return 0
